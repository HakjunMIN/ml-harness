...
- Explicit human approval for the exact patch or release action, referencing the AIDM manifest checksum.
...
5. Invoke `human-review` to display the safe in-agent evidence summary before requesting final review.
6. Verify human approval is explicit, current, scoped to this run and patch, and references the manifest checksum.
7. If any item is missing, say `fail closed` and list the missing evidence.
8. Agents must not deploy, must not merge, and must not edit customer systems. Agents may only prepare a human-reviewed patch request.
9. Reject `research-summary.json`, `state.json`, `journal.jsonl`, or any research-loop terminal
...
| No human approval for the manifest checksum | Fail closed; do not release. |
...
- Gate summary: baseline path, AIDM artifacts, AIDD artifacts, compile result, human approval reference, manifest checksum, and final decision.
