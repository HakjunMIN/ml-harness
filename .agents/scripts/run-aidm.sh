...
usage() {
  cat >&2 <<'USAGE'
usage: run-aidm.sh --dataset PATH --run-dir DIR [--catalog PATH] [--proposal PATH] [--legacy-predictions PATH] [--folds N] [--minimum-improvement X] [--max-plant-regression X] [--top-single-candidates N] [--seed N]
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --catalog|--proposal|--legacy-predictions)
